...
const inputBase = 'w-full bg-white dark:bg-[#252945] border border-[#DFE3FA] dark:border-[#252945] rounded px-5 py-4 font-bold text-[12px] tracking-[-0.25px] text-[#0C0E16] dark:text-white placeholder:text-[#888EB0]/50 focus:outline-none focus:border-[#7C5DFA] transition-colors duration-200';
const labelBase = 'text-[#7E88C3] dark:text-[#DFE3FA] font-medium text-[12px]';
const sectionTitle = 'text-[#7C5DFA] font-bold text-[12px] tracking-[-0.25px] mb-6';
...
export default function InvoiceForm({ invoice, onClose }: InvoiceFormProps) {
  ...
  // Render helper — plain function, not a component, to avoid state reset on re-render.
    ...
    return (
      <div key={path} className="flex flex-col gap-[10px]">
        <div className="flex justify-between items-center">
          ...
          {err && <span className="text-[#EC5757] font-medium text-[10px]">{err}</span>}
        </div>
        <input
          id={path}
          type={type}
  ...
  return (
    <>
      ...
      {/* Panel — sits above overlay and sidebar on desktop starts after sidebar */}
      <div
        className="fixed top-[72px] md:top-0 bottom-0 left-0 z-[60]
          w-full md:max-w-[616px] md:left-[103px]
      ...
      >
        {/* Scrollable body */}
        <div className="flex-1 overflow-y-auto px-6 md:px-14 pt-10 md:pt-14 pb-8">
          ...
          <div className="flex flex-col gap-10">
              ...
              <div className="flex flex-col gap-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="flex flex-col gap-[10px]">
                    <div className="flex justify-between items-center">
                      ...
                      {errors.createdAt && (
                        <span className="text-[#EC5757] font-medium text-[10px]">{errors.createdAt}</span>
                      )}
                    </div>
                    <input
                      id="createdAt"
                ...
                <div className="flex flex-col gap-[10px]">
                  <div className="flex justify-between items-center">
                    ...
                    {errors.description && (
                      <span className="text-[#EC5757] font-medium text-[10px]">{errors.description}</span>
                    )}
                  </div>
                  <input
                    id="description"
            ...
            <div>
              ...
              {form.items.length > 0 && (
                <div className="hidden md:grid md:grid-cols-[1fr_48px_100px_64px_16px] md:gap-x-4 md:mb-4">
              ...
              <div className="flex flex-col gap-12 md:gap-4">
                {form.items.map((item, i) => {
                  ...
                  return (
                    <div key={i} className="flex flex-col md:grid md:grid-cols-[1fr_48px_100px_64px_16px] md:items-center md:gap-x-4 gap-6">
                      <div className="flex flex-col gap-[10px]">
                      ...
                      <div className="grid grid-cols-[48px_1fr_64px_16px] md:contents items-center gap-x-4">
                        <div className="flex flex-col gap-[10px]">
                        ...
                        <button
                          type="button"
                          className="flex items-end justify-center pb-4 md:pb-0 md:items-center text-[#888EB0] hover:text-[#EC5757] transition-colors duration-200"
              ...
              {errors.items && (
                <p className="text-[#EC5757] font-medium text-[10px] mt-3">{errors.items}</p>
              )}

              <button
                type="button"
                onClick={addItem}
                className="mt-6 w-full py-4 rounded-full font-bold text-[12px] tracking-[-0.25px] bg-[#F9FAFE] dark:bg-[#252945] text-[#7E88C3] hover:bg-[#DFE3FA] dark:hover:bg-[#1E2139] transition-colors duration-200"
              >
                + Add New Item
              </button>
            </div>
            ...
            {Object.keys(errors).length > 0 && (
              <p className="text-[#EC5757] font-medium text-[10px]" role="alert">
        ...
        {/* Sticky action bar */}
        <div className="shrink-0 px-6 md:px-14 py-5 bg-white dark:bg-[#1E2139] shadow-[0_-8px_24px_rgba(72,84,159,0.1)] dark:shadow-[0_-8px_24px_rgba(0,0,0,0.4)] flex items-center gap-2">
          {!isEdit ? (
            <>
              <button
                type="button"
                className="inline-flex items-center justify-center rounded-full font-bold text-[12px] tracking-[-0.25px] px-6 py-[17px] bg-[#F9FAFE] text-[#7E88C3] hover:bg-[#DFE3FA] dark:bg-[#252945] dark:text-[#DFE3FA] dark:hover:bg-[#DFE3FA] dark:hover:text-[#0C0E16] transition-colors duration-200 border-0 cursor-pointer"
                onClick={onClose}
              >
                Discard
              </button>
              <div className="flex items-center gap-2 ml-auto">
                <button
                  type="button"
                  className="inline-flex items-center justify-center rounded-full font-bold text-[12px] tracking-[-0.25px] px-6 py-[17px] bg-[#373B53] text-[#DFE3FA] hover:bg-[#0C0E16] transition-colors duration-200 border-0 cursor-pointer"
                ...
                <button
                  type="button"
                  className="inline-flex items-center justify-center rounded-full font-bold text-[12px] tracking-[-0.25px] px-6 py-[17px] bg-[#7C5DFA] text-white hover:bg-[#9277FF] transition-colors duration-200 border-0 cursor-pointer"
            ...
            <>
              ...
              <button
                type="button"
                className="inline-flex items-center justify-center rounded-full font-bold text-[12px] tracking-[-0.25px] px-6 py-[17px] bg-[#7C5DFA] text-white hover:bg-[#9277FF] transition-colors duration-200 ml-auto border-0 cursor-pointer"
                onClick={() => handleSubmit(false)}
              >
                Save Changes
              </button>
